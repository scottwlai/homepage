...
/**
 * Route handler for the GET method on the /certifications endpoint
...
router.get('/', cors(corsOptions), async(req, res, next) => {
  ...
  // selects the Certifications collection in the database
  ...
  // executes the query
  const data = await certifications.find({}, { sort: { date: -1 } }).limit(limit).toArray();
  // sends the client a JSON response containing the resulting array
  res.json(data);
});

// exports the router object to be mounted to /certifications in app.js
